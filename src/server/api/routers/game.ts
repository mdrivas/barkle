...
export const gameRouter = createTRPCRouter({
  getDailyBreeds: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        // potentially change this to be more robust and store individual breeds in a table so no parsing is needed, if you're trying to optimize server performance, im pretty sure parsing is more expensive than the multiple db queries.
