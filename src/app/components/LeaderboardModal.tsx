...
export function LeaderboardModal({
  ...
  const [selectedUser, setSelectedUser] = useState<{ userId: string | null; tempId: string | null } | null>(null);

  useEffect(() => {
    if (open) {
      setMode(source ?? defaultMode);
    }
  }, [open, source, defaultMode]);

  const handleUserClick = (userId: string | null, tempId: string | null) => {
    setSelectedUser({ userId, tempId });
  };
  ...
  return (
    <>
      ...
      <LevelSystemModal 
        ...
        tempId={selectedUser?.tempId ?? null}
      />
    </>
  );
}
